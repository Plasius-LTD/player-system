...
export function promoteUnreleasedChangelog(content, options) {
  ...
  const tail = nextHeadingMatch
    ? afterHeadingWithoutLeadingNewline.slice(nextHeadingMatch.index).trimStart()
    : "";

  const promotedBody =
    unreleasedBody.trim().length > 0
...
async function main() {
  ...
  if (!filePath || !nextVersion) {
    console.error(
      "Usage: node scripts/promote-unreleased-changelog.mjs <file> <version> [repository]",
    );
    process.exit(1);
  }
